...
\vspace{-1em}
\scriptsize
```{r corr_table, echo=FALSE, message=FALSE, warning=FALSE}
library(kableExtra)
library(formattable)

# Redondear a dos decimales
cor_data <- round(as.data.frame(PCAtodas$var$cor), 2)
names(cor_data) <- paste0("C", 1:ncol(cor_data))  # Encabezados compactos

# Función para formato condicional
color_cor <- function(x) {
  cell_spec(x, "latex",
    bold = ifelse(abs(x) > 0.5, TRUE, FALSE),
    color = case_when(
      x > 0.5 ~ "green!50!black",
      x < -0.5 ~ "red",
      TRUE ~ "gray"
    )
  )
...
kable(cor_data %>% mutate(across(everything(), color_cor)),
      caption = "Correlaciones variables-componentes",
      format = "latex",
      booktabs = TRUE,
      align = "c",
      escape = FALSE) %>%
  ...
  footnote(
    general = "C1-C6: Componentes 1-6. |r| > 0.5 en negrita",
...
knitr::kable(var_results, "latex", booktabs = TRUE, 
             ...
             label = "pca_results") %>%
  kableExtra::kable_styling(latex_options = c("striped", "scale_down", "hold_position"), font_size = 6)
...
# Crear un dataframe con todas las métricas redondeadas a 2 decimales
var_coord <- as.data.frame(round(pcaRHINTce$var$coord, 2))
var_contrib <- as.data.frame(round(pcaRHINTce$var$contrib, 2))
var_cos2 <- as.data.frame(round(pcaRHINTce$var$cos2, 2))

# Combinar solo las dimensiones 3 y 4
var_results <- data.frame(
  Variable = rownames(var_coord),
...
# Mostrar tabla en formato compacto
knitr::kable(var_results, format = "latex", booktabs = TRUE,
             caption = "Resultados del PCA: Coordenadas, Contribución y Calidad de Representación",
             label = "pca_results") %>%
...
\vspace{-0.5em}
```{r comprativa, echo=FALSE, warning=FALSE}
library(kableExtra)
library(dplyr)

# Asumiendo que tienes tres objetos ACP: PCAtodas, res.pca y pcaRHINTce
# Extraer datos de varianza explicada de cada ACP
...
kable(comparativa_varianza, 
      format = "latex",
      booktabs = TRUE,
      caption = "Comparativa de varianza explicada entre los tres ACP",
      digits = 2) %>%
  kable_styling(latex_options = c("hold_position", "striped"),
                font_size = 5) %>%
  pack_rows("ACP 1", 1, 7) %>%
  pack_rows("ACP 2", 8, 8+nrow(varianza2)-1) %>%
  pack_rows("ACP 3", 8+nrow(varianza2), 8+nrow(varianza2)+nrow(varianza3)-1) %>%
  footnote(general = "Comparación de la varianza explicada por componentes principales en tres análisis diferentes",
           number = paste("Fecha:", Sys.Date()),
           threeparttable = TRUE)
```
