...
# Preparar los datos
varianza1 <- as.data.frame(PCAtodas$eig[1:8, ]) %>% 
  mutate(ACP = "ACP 1", Componente = paste("Comp", 1:8))

varianza2 <- as.data.frame(res.pca$eig[1:3, ]) %>% 
  mutate(ACP = "ACP 2", Componente = paste("Comp", 1:3))

varianza3 <- as.data.frame(pcaRHINTce$eig[1:8, ]) %>% 
  mutate(ACP = "ACP 3", Componente = paste("Comp", 1:8))

# Combinar todo
comparativa_varianza <- bind_rows(varianza1, varianza2, varianza3) %>% 
  mutate(across(where(is.numeric), round, 2))  # Redondear

# Guardar ACP para usar en group_rows() y resaltado, pero no pasarlo a la tabla
grupos <- comparativa_varianza$ACP

# Seleccionar columnas a mostrar, omitiendo ACP
comparativa_varianza <- comparativa_varianza %>%
  select(-ACP) %>%
  select(Componente, eigenvalue, `percentage of variance`, `cumulative percentage of variance`)

# Cambiar nombres para la tabla
names(comparativa_varianza) <- c("Componente", "Valor propio", "% Varianza", "% Acumulado")

# Crear tabla
tabla <- kable(comparativa_varianza,
               format = "html",
  ...
  group_rows("ACP 1", 1, 8, background = "#e6f7ff") %>%
  group_rows("ACP 2", 9, 11, background = "#f9e6ff") %>%
  group_rows("ACP 3", 12, 19, background = "#e6ffe6") %>%
  footnote(
...
# Resaltar máximos % Varianza por grupo
indices_max_var <- comparativa_varianza %>%
  mutate(ACP = grupos) %>%
  group_by(ACP) %>%
